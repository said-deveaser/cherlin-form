...
export const createField = <FormValues extends Record<string, any>>({formValuesStore, formErrorsStore}: CreateFieldParams) =>
  function <Value = string, Error = string, Props = Record<string, any>>(props: FieldProps<Value, Error, FormValues, Props>) {
    const RenderComponent = useMemo(() => React.memo(Field), []) as FC<FieldProps<Value, Error, FormValues, Props> & CreateFieldParams>
    const validate = props.validate ? useCallback(props.validate, []) : undefined
    const render = useCallback(props.render, [])

    return <RenderComponent props={props.props} name={props.name} validate={validate} render={render} formValuesStore={formValuesStore} formErrorsStore={formErrorsStore} />
  }
    ...
    <React.Fragment>
      <RenderComponent
        field={useMemo(
          ...
          [value, setValueToFormValuesStore, {...props.props}],
        )}
        form={useMemo(
          () => ({
