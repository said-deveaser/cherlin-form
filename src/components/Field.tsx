...
export const createField = <FormValues extends Record<string, any>>({formValuesStore, formErrorsStore}: CreateFieldParams) =>
  function <Value = string, Error = any>(props: FieldProps<Value, Error, FormValues>) {
    ...
    const render = useMemo(() => props.render, [])
    const FieldComponent = useCallback(React.memo(Field) as FC<FieldProps<Value, Error, FormValues> & CreateFieldParams>, [])


    return <FieldComponent name={props.name} validate={validate} render={render} formValuesStore={formValuesStore} formErrorsStore={formErrorsStore} />
  }

const Field = /* React.memo( */ function <Value, Error, FormValues>({formValuesStore, formErrorsStore, ...props}: FieldProps<Value, Error, FormValues> & CreateFieldParams) {
  const initialFormFieldValue = formValuesStore.getFieldValue(props.name) as Value
  const formValues = formValuesStore.getFormValues() as FormValues
  ...
  // const RenderComponent = useMemo(() => props.render, [value, error, touched, setValueToFormValuesStore])
  const RenderComponent = useCallback(React.memo(props.render), [] )
  return (
    <React.Fragment>
      <RenderComponent
        field={useMemo(
